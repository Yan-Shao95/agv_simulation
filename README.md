...
三种车型共用 `/cmd_vel`、Gazebo 世界和传感器桥接。麦克纳姆和阿克曼由各自的 Gazebo 驱动插件直接消费 `/cmd_vel`；以下实际启动的控制链仅用于默认差动舵轮车型：
...
- 对正后继续执行舵角闭环，并使用进入/退出滞环避免在容差边界反复启停。
- 使用限幅积分补偿克服静摩擦导致的残余转角误差，解决横移时总成停在目标角度附近但车体不走的问题。
- 使用仿真时间限制驱动加减速度，使不同实时因子下的控制行为保持一致。
- 在车体自转时计入同一总成内外轮的速度差，避免轮组偏离切向目标。

Gazebo 世界真实位姿已经验证前进、后退、横移、斜行和原地自转。原地自转测试的平移漂移约 2 mm；零速度保持测试中，模型位置和姿态未发生变化。
...
先在键盘控制窗口按 `k` 停车，再按 `Ctrl+C` 退出键盘节点；随后在启动仿真的终端按 `Ctrl+C`。退出会停止 Gazebo、车辆节点和 ros_gz_bridge。当前部分 Python 控制节点在接收中断时可能打印 `KeyboardInterrupt` 回溯和退出码 `-2`，但进程仍会结束；这是关闭阶段的日志问题，不影响运行中的控制功能。
